...
　　  
Twitter/Facebookアカウントによるログインも可能です。
（ログイン後は、タイトルバーの「設定変更」から退会処理をすることで、SNSアカウントの情報をDBから削除できます）
...
+ パスワード: administrator
　　
管理者自身のアカウント設定は変えられません｡(アカウント名､パスワード､画像､ロック状態など)  
...
+ ユーザー画像・ユーザー名をクリック  
ユーザープロフィール画面に移行。  
フォロー・アンフォロー、プロフィールの確認、フォロワー数の確認などが可能。   
検索窓を使うと、表示中のユーザーのツイートだけに限定した検索がかけられる。  
<br />

+ ツイートされた文字要素、ツイート時間をクリック  
ツイート詳細画面へ移行。ツイートへの返信ができる。  
（編集可能なツイートの場合、ツイートの文字要素をクリックすると編集・削除画面へ飛びます。ツイート詳細画面へ移る場合は、ツイート時間をクリックしてください。）  
<br />

+ リアクションボタン  
ツイートの近くにマウスを合わせると、いいね！などのアイコンを含むボタンが現れます。  
<br />
...
+ タイトルバー  
左上のロゴをクリックすることで、どの画面にいても常にトップページに戻れます。ログアウトも常に可能。  
他にも検索窓、自分のプロフィール表示画面とプロフィール編集画面へのリンクがあります。  
<br />
